...
file_path = r'C:\Users\tinajero\Desktop\codes_Cristopher\code2_LCMS_script1\code1.xlsx'
...
text_path = r'C:\Users\tinajero\Desktop\codes_Cristopher\code2_LCMS_script1\decoded_output.txt'
...
def extract_compound_block(compound, lines):
    ...
    Returns the lines that contain the string 'code2_' (used to identify each vial in this format).
    """
    ...
    for j in range(start_index + 1, len(lines)):
        ...
        if re.search(r'Compound\s+\d+:', lines[j], re.IGNORECASE):
            break
        # Only consider lines containing the phrase "code2_"
        if "code2_" in lines[j].lower():
